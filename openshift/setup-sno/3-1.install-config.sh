...
# installation_directory_sno 폴더 생
mkdir -p /root/installation_directory_sno
cd /root/installation_directory_sno

# 설정파일 생성
CONFIG_FILE="/root/installation_directory_sno/install-config.yaml"
cat /dev/null > /root/installation_directory_sno/install-config.yaml
...
networking:
  ...
  - cidr: 192.168.0.0/24 #SNO 설치 시 주석 해제
...
# manifests 생성
openshift-install create manifests --dir installation_directory_sno/

# master 컨테이너 배포 설정 false 처리, SNO 설치 시 주석
#sed -i 's/true/false/' /root/installation_directory_sno/manifests/cluster-scheduler-02-config.yml

# ignition config 파일 생성
cd /root
#openshift-install create ignition-configs --dir installation_directory_sno/
openshift-install create single-node-ignition-config --dir installation_directory_sno/ # SNO 설치 시 주석 해제
