...
| 구분 | 목적 | 명령어 |
| :--- | :--- | :--- |
| **n8n CLI** | 모든 워크플로를 컨테이너 `/tmp`에 내보내기 | `docker exec n8nio-n8n n8n export:workflow --all --output=/tmp/workflows.json` |
| **Docker CLI** | 컨테이너의 워크플로를 호스트로 복사 | `docker cp n8nio-n8n:/tmp/workflows.json ./n8n_backup/workflows.json` |
| **n8n CLI** | 모든 자격 증명을 컨테이너 `/tmp`에 내보내기 | `docker exec n8nio-n8n n8n export:credentials --all --output=/tmp/credentials.json` |
| **Docker CLI** | 컨테이너의 자격 증명을 호스트로 복사 | `docker cp n8nio-n8n:/tmp/credentials.json ./n8n_backup/credentials.json` |
...
| 구분 | 목적 | 명령어 |
| :--- | :--- | :--- |
| **Docker CLI** | 호스트의 워크플로를 컨테이너 `/tmp`로 복사 | `docker cp ./n8n_backup/workflows.json n8nio-n8n:/tmp/workflows.json` |
| **n8n CLI** | 워크플로 가져오기 실행 | `docker exec n8nio-n8n n8n import:workflow --input=/tmp/workflows.json` |
| **Docker CLI** | 호스트의 자격 증명을 컨테이너 `/tmp`로 복사 | `docker cp ./n8n_backup/credentials.json n8nio-n8n:/tmp/credentials.json` |
| **n8n CLI** | 자격 증명 가져오기 실행 | `docker exec n8nio-n8n n8n import:credentials --input=/tmp/credentials.json` |
...
| 구분 | 목적 | 명령어 |
| :--- | :--- | :--- |
| **n8n CLI** | 컨테이너 내 임시 파일 모두 삭제 | `docker exec n8nio-n8n rm /tmp/workflows.json /tmp/credentials.json` |
